...
int *pyramid[] = {row1, row2, row3, row4, row5, row6, row7, row8, row9, row10, row11, row12, row13, row14, row15};

int max(int a, int b) {
    return a > b ? a : b;
}

int search(int current_x, int current_y) {
    if (current_y == 14) {
        return pyramid[current_y][current_x];
    }

    int left = search(current_x, current_y+1);
    int right = search(current_x + 1, current_y + 1);

    return pyramid[current_y][current_x] + max(left, right);
}

int main() {
    int highest = search(0, 0);
    printf("%d", highest);
}